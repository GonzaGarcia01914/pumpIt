...
class TransactionAuditLogger {
  ...
  Future<Excel> _getOrCreateExcel() async {
    ...
    if (_sheet == null) {
      _sheet = _excel!['Trades'] = _excel!.sheets['Sheet1'] ?? _excel!.sheets.values.first;
    }
  ...
  Future<void> _writeCriteriaHeader(AutoInvestState state) async {
    ...
    // Título de sección
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: _currentRow))
        .value = TextCellValue('CRITERIOS DE MERCADO');
    _applyHeaderStyle(sheet, CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: _currentRow));
    sheet.merge(
    ...
    for (var i = 0; i < criteria.length; i++) {
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: _currentRow))
          .value = TextCellValue(criteria[i][0]);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: _currentRow))
          .value = TextCellValue(criteria[i][1]);
      _applyCriteriaStyle(sheet, CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: _currentRow));
      _applyCriteriaStyle(sheet, CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: _currentRow));
      _currentRow++;
    }
    ...
    for (var i = 0; i < headers.length; i++) {
      final cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: i, rowIndex: _currentRow));
      cell.value = TextCellValue(headers[i]);
      _applyHeaderStyle(sheet, CellIndex.indexByColumnRow(columnIndex: i, rowIndex: _currentRow));
    }
    _currentRow++;
  ...
  Future<void> logBuyFromFeatured({
    ...
    unawaited(_logBuyFromFeaturedInternal(
      coin: coin,
      signature: signature,
      entrySol: entrySol,
      mode: mode,
  ...
  Future<void> logSellFromPosition({
    ...
    // ⚡ ASÍNCRONO: No esperar, actualizar en background
    unawaited(_logSellFromPositionInternal(
      position: position,
      signature: signature,
  ...
  Future<void> _logSellFromPositionInternal({
    ...
    final finalPnlSol = pnlSol ?? (realizedExitSol != null ? (realizedExitSol - position.entrySol) : null);
    final finalPnlPercent = pnlPercent ?? (position.entrySol > 0 && finalPnlSol != null 
    ...
    final row = [
      ...
      (finalPnlSol != null && position.entryFeeSol != null && position.exitFeeSol != null
          ? (finalPnlSol - position.entryFeeSol! - position.exitFeeSol!)
          : null)?.toStringAsFixed(8) ?? '',
    ];
    
    for (var i = 0; i < row.length; i++) {
      final cellIndex = CellIndex.indexByColumnRow(columnIndex: i, rowIndex: _currentRow);
      final cell = sheet.cell(cellIndex);
      cell.value = TextCellValue(row[i]);
      
      // Aplicar color según win/loss
      if (i == 11 && finalPnlSol != null) { // Columna PnL SOL
        if (finalPnlSol >= 0) {
      ...
      if (i == 12 && finalPnlPercent != null) { // Columna PnL %
  ...
  Future<void> _updateStatistics(Excel excel, AutoInvestState state) async {
    ...
    final totalNetPnl = allTrades.fold<double>(0, (sum, p) => sum + (p.netPnlSol ?? p.pnlSol));
    
    int row = 0;
    
    // Título
    statsSheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row))
        .value = TextCellValue('ESTADÍSTICAS GENERALES');
    _applyHeaderStyle(statsSheet, CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row));
    statsSheet.merge(
    ...
    for (var i = 0; i < statHeaders.length; i++) {
      statsSheet.cell(CellIndex.indexByColumnRow(columnIndex: i, rowIndex: row))
          .value = TextCellValue(statHeaders[i]);
      _applyHeaderStyle(statsSheet, CellIndex.indexByColumnRow(columnIndex: i, rowIndex: row));
    }
    row++;
    ...
    for (var stat in generalStats) {
      ...
      statsSheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row))
    ...
    row += 2;
    statsSheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row))
        .value = TextCellValue('ESTADÍSTICAS POR CRITERIOS');
    _applyHeaderStyle(statsSheet, CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row));
    statsSheet.merge(
      CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row),
      CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row),
    ...
    for (final entry in criteriaGroups.entries) {
      ...
      final groupWinRate = groupTotal > 0 ? (groupWins / groupTotal * 100) : 0.0;
      final groupPnl = trades.fold<double>(0, (sum, p) => sum + p.pnlSol);
      final groupNetPnl = trades.fold<double>(0, (sum, p) => sum + (p.netPnlSol ?? p.pnlSol));
      
      row++;
      statsSheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row))
          .value = TextCellValue('Grupo de Criterios');
      _applyCriteriaStyle(statsSheet, CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row));
      statsSheet.merge(
        CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row),
        CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row),
      );
